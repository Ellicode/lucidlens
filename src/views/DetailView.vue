...
import { GlobeAltIcon, InformationCircleIcon } from '@heroicons/vue/16/solid'
...
const deleteArticleLoading = ref(false)

const deleteArticle = async () => {
  if (article.value) {
    deleteArticleLoading.value = true
    await deleteDoc(doc(db, 'articles', route.params.id as string))
    comments.value.forEach((comment) => {
      deleteDoc(doc(db, 'articles', route.params.id as string, 'comments', comment.id))
    })
    deleteArticleLoading.value = false
        ...
        <div class="flex-1">
          ...
          <p class="text-neutral-400">
            ...
            <button
              ...
              class="inline-flex cursor-pointer items-center gap-1 text-neutral-500 hover:text-red-500"
            >
              Delete
            </button>
          </p>
    ...
    >
      <div class="max-h-9/10 w-full max-w-2xl overflow-auto rounded-md bg-white p-7 shadow-lg">
        <div class="mb-5 flex items-center">
          <h3 class="font-serif text-2xl">Delete article</h3>
          <button
        ...
        >
          <template v-if="deleteArticleLoading">
            <ProgressIndicator size="xs" class="text-white" />
          </template>
          <template v-else>Delete article</template>
        </button>
      </div>
    </div>
    <div
